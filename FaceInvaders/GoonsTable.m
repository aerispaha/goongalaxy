...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cellFaceData.isActive intValue] == 1) {
        [tableView selectRowAtIndexPath:indexPath animated:NO scrollPosition:UITableViewScrollPositionNone];
        cell.detailTextLabel.textColor = [UIColor yellowColor];
        NSLog(@"ACTIVE %@ returned at index:%d", cellFaceData.name, indexPath.row);
    }else{
        [tableView deselectRowAtIndexPath:indexPath animated:NO];
        cell.detailTextLabel.textColor = [UIColor colorWithRed:0.22 green:0.33 blue:0.53 alpha:1];
        NSLog(@"inactive %@ returned at index:%d", cellFaceData.name, indexPath.row);
    }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSError *err;
    NSString *fdFilePath = /*fd.filePath*/ [[FaceData faceDataDir] stringByAppendingPathComponent:cellFaceData.fileName];
...
- (void)removeIndexFromActivePlayers:(NSIndexPath *)index{
...
- (void)addIndexToActivePlayers:(NSIndexPath *)index{
    ...
    cellFaceData.isActive = num; /*(YES, is active)*/ [num release];
